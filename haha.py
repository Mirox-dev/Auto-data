...
def create_ul_report(target_path, is_file, hash_type, output_name, workers_count):
    ...
    # 1 финальная строка (Информационно-удостоверяющий лист)
    total_rows = 1 + 1 + 1 + 1 + 1 + n + 1 + m + 1
    ...
    # Колонки для основной части (в 5-й строке): [4.92, 3.79, 4.25, 3.71]
    col_widths = [4.92, 3.79, 4.25, 3.71]  # см
    # Но в первой/четвёртой/последней строке используются другие комбинации:
    ...
    # используем колонки для шаблона: [4.92, 3.79, 4.25, 3.71] как базу
    ...
    set_cell_text(table.rows[r].cells[1], "")  # визуально занимают остальные колонки
    ...
    set_cell_text(table.rows[r].cells[3], "Номер последнего изменения (версии)", bold=True)
    r += 1
    ...
    set_cell_text(table.rows[r].cells[1], "")
    set_cell_text(table.rows[r].cells[2], "")
    set_cell_text(table.rows[r].cells[3], "")
    r += 1
    ...
    set_cell_text(table.rows[r].cells[1], "Дата и время последнего изменения файла", bold=True)
    set_cell_text(table.rows[r].cells[2], "Размер файла, байт", bold=True)
    set_cell_text(table.rows[r].cells[3], "Значение контрольной суммы", bold=True)
    r += 1
    ...
    for name, mod_dt_full, size, checksum in file_rows:
        ...
        set_cell_text(table.rows[r].cells[1], mod_dt_full)
        set_cell_text(table.rows[r].cells[2], size)
        set_cell_text(table.rows[r].cells[3], checksum)
        r += 1

    # После всех файлов: строка с заголовками для подписи
    set_cell_text(table.rows[r].cells[0], "Характер работы", bold=True)
    set_cell_text(table.rows[r].cells[1], "Фамилия", bold=True)
    set_cell_text(table.rows[r].cells[2], "Подпись", bold=True)
    set_cell_text(table.rows[r].cells[3], "Дата подписания", bold=True)
    r += 1
    ...
    for i in range(m):
        ...
        set_cell_text(table.rows[r].cells[1], "")
        set_cell_text(table.rows[r].cells[2], "")
        set_cell_text(table.rows[r].cells[3], latest_date_str if latest_date_str else "")
        r += 1
    ...
    set_cell_text(table.rows[r].cells[0], "Информационно-удостоверяющий лист")
    set_cell_text(table.rows[r].cells[1], base_name_no_ext)
    # Для 3 и 4 ячеек добавим сверху spacing_before ~0.5 см (см. заметку в начале)
    set_cell_text(table.rows[r].cells[2], "Лист", spacing_before_cm=0.5)
    set_cell_text(table.rows[r].cells[3], "Листов", spacing_before_cm=0.5)
    r += 1
...
class App:
    def __init__(self, root):
        ...
        # простой popup-выбор реализуем двумя кнопками-альтернативами (можно заменить Combo)
        Button(root, text="MD5", command=lambda: self.hash_choice.set("MD5")).grid(row=1, column=1, sticky=W, padx=6)
        Button(root, text="CRC32", command=lambda: self.hash_choice.set("CRC32")).grid(row=1, column=2, sticky=W, padx=6)
        self.hash_label = Label(root, textvariable=self.hash_choice)
        self.hash_label.grid(row=1, column=3, padx=6)
